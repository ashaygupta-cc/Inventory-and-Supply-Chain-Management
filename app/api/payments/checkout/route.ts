...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isStripeConfigured()) {
      logger.info("Stripe not configured. Mocking successful payment session...");
      const body: CreateCheckoutInput = await request.json();
      const { type, id, successUrl } = body;
        ...
        if (order && order.paymentStatus !== "paid") {
          await prisma.order.update({
            ...
            data: {
              ...
              stripePaymentIntentId: "pi_mock_" + fakeSessionId,
              updatedAt: new Date(),
            },
          });
          ...
          const { ensureInvoiceForPaidOrder } = await import("@/prisma/invoice");
          await ensureInvoiceForPaidOrder(id, order.total).catch(() => {});
        }
      } else if (type === "invoice") {
        const invoice = await prisma.invoice.findUnique({ where: { id }, include: { order: { include: { items: true } } } });
      ...
      const response: CheckoutSessionResponse = {
        ...
        url: successUrl || `${baseUrl}/${type}s/${id}?payment=success&session_id=${fakeSessionId}`
      };
